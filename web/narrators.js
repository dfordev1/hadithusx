...
try {
  ...
  $("#narrator-results").addEventListener("click", (event) => { const pageButton = event.target.closest("[data-page]"); if (pageButton && !pageButton.disabled) { search(Number(pageButton.dataset.page)).catch(showError); return; } const evidence = event.target.closest("[data-show-evidence]"); if (evidence) showEvidence(evidence.closest("[data-cluster-id]")).catch(showError); });
